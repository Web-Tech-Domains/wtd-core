...
final class ErrorPageRenderer
{
    public function render(Throwable $throwable, int $status, bool $debug): string
    {
        ...
        $details = $debug
            ? '<section class="debug"><div class="debug-head"><span>Exception</span><code>' . $this->escape($throwable::class) . '</code></div><h2>Debug details</h2><pre>' . $this->escape($throwable->getFile() . ':' . $throwable->getLine() . "\n\n" . $throwable->getTraceAsString()) . '</pre></section>'
            : '';
    ...
    {
        return <<<'CSS'
:root{color-scheme:light;--bg:#f6f8fb;--panel:#fff;--ink:#102032;--muted:#64748b;--line:#dbe3ec;--blue:#2563eb;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}*{box-sizing:border-box}html{overflow-x:hidden}body{min-width:320px;margin:0;overflow-x:hidden;background:var(--bg);color:var(--ink)}a{color:inherit;text-decoration:none}.topbar{min-height:72px;display:flex;align-items:center;justify-content:space-between;gap:18px;padding:0 max(24px,calc((100vw - 1120px)/2));border-bottom:1px solid var(--line);background:rgba(255,255,255,.96);backdrop-filter:blur(16px)}.brand{display:flex;align-items:center;gap:12px;font-weight:800;color:#172554}.brand-logo{width:38px;height:38px;display:block;flex:0 0 auto}nav{display:flex;flex-wrap:wrap;gap:10px}nav a{min-height:38px;display:inline-flex;align-items:center;padding:0 12px;border-radius:8px;color:var(--muted);font-size:14px;font-weight:800}nav a:hover{background:#eef6ff;color:var(--blue)}.wrap{width:min(1120px,calc(100% - 40px));margin:0 auto}.hero{min-height:calc(100vh - 72px);display:grid;grid-template-columns:minmax(0,1fr)minmax(280px,360px);gap:34px;align-items:center;padding:70px 0}.badge{width:fit-content;display:inline-flex;align-items:center;min-height:32px;padding:0 12px;border:1px solid #bfdbfe;border-radius:999px;background:#eff6ff;color:#1d4ed8;font-size:13px;font-weight:900}h1{max-width:760px;margin:18px 0 14px;font-size:clamp(40px,6vw,68px);line-height:1.04;letter-spacing:0;overflow-wrap:anywhere}p{max-width:680px;margin:0;color:var(--muted);font-size:18px;line-height:1.7}.actions{display:flex;flex-wrap:wrap;gap:12px;margin-top:28px}.actions a{min-height:46px;display:inline-flex;align-items:center;justify-content:center;padding:0 16px;border:1px solid var(--line);border-radius:8px;background:#fff;font-weight:900}.actions .primary{border-color:var(--blue);background:var(--blue);color:#fff;box-shadow:0 16px 34px rgba(37,99,235,.22)}.panel{border:1px solid var(--line);border-radius:8px;background:var(--panel);padding:24px;box-shadow:0 20px 52px rgba(16,32,50,.08)}.panel span{display:block;color:var(--muted);font-size:14px;font-weight:800}.panel strong{display:block;margin:8px 0 12px;font-size:64px;line-height:1;color:var(--blue)}.panel p{font-size:15px}.debug{max-width:100%;margin:0 0 56px;border:1px solid var(--line);border-radius:8px;background:#fff;padding:22px}.debug-head{display:grid;gap:8px;margin-bottom:16px}.debug-head span{color:var(--muted);font-size:13px;font-weight:900;text-transform:uppercase}.debug code{max-width:100%;display:block;overflow:auto;border:1px solid var(--line);border-radius:8px;background:#f8fafc;padding:10px 12px;color:#334155;font-size:13px}.debug h2{margin:0 0 14px;font-size:20px}.debug pre{max-width:100%;overflow:auto;margin:0;background:#0f172a;color:#e2e8f0;padding:16px;border-radius:8px;font-size:13px;line-height:1.6}@media(max-width:820px){.topbar{align-items:flex-start;flex-direction:column;padding:14px 20px}.hero{min-height:auto;grid-template-columns:1fr;padding:48px 0}.panel{max-width:100%}.actions a{width:100%}}@media(max-width:460px){.wrap{width:min(100% - 28px,1120px)}h1{font-size:38px}.panel strong{font-size:56px}}
CSS;
    }
